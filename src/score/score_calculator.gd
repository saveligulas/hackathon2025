...
# In src/score/scorecalculator.gd
...
func calculate_score_with_state(slot_grid_result: Array, game_state) -> Dictionary:
    ...
    var base_points = 0
    var final_multiplier = 0  # CHANGED: Start at 0
    ...
    # Free-flowing lines (now checks regardless of pattern matches)
    print("\n[FREE FLOW] Checking free-flowing lines...")
    ...
    for result in free_flow_results:
        if result.matched:
            ...
            var calculated_mult = 1
            var flow_base_points = 0
            var flow_base_mult = 1
            
            # Calculate what each symbol's points and mult are AFTER effects
            for position in result.positions:
                var symbol_context = apply_symbol_effects(symbol, game_state)
                flow_base_points += symbol_context.points
                calculated_mult += symbol_context.mult - 1
                flow_base_mult = symbol_context.mult
            
            # Add to totals
            base_points += flow_base_points
            final_multiplier += flow_base_mult
            
            print("[DEBUG PREVIEW] Flow points: %d, Flow mult: %d | Total points so far: %d, Total mult so far: %d" % [flow_base_points, calculated_mult, base_points, final_multiplier])
    ...
    print("[DEBUG FINAL] base_points: %d, final_multiplier: %d" % [base_points, final_multiplier])
    print(divider + "\n")
...
func calculate_free_flow_score_with_effects(match_result: Dictionary, slot_grid_result: Array, game_state) -> int:
    print("[FREE FLOW SCORING]")
    var points = 0
    var mult = 1  # Start at 1 (base multiplier)
    ...
    # Calculate points and add multipliers
    for position in match_result.positions:
        var symbol_context = apply_symbol_effects(symbol, game_state)
        points += symbol_context.points
        mult += symbol_context.mult - 1  # ADD the bonus mult, not multiply
    
    print("    After effects: %d pts, %d mult total" % [points, mult])
    
    var final_score = points * mult
    print("    After global effects: %d pts x %d mult = %d" % [points, mult, final_score])
